...
func cottageCollector(db *pgxpool.Pool) error {
	...
	for i := 1; i > 0; i++ {
		URL := fmt.Sprintf("https://domovita.by/vitebsk/houses/sale?page=%d", i)
		err := c.Visit(URL)
		if err != nil {
			if err.Error() != "Not Found" {
				logrus.Printf("an error uccured in cottageCollector while visiting URL %s", err.Error())
			}
			break
		}
	}
...
func writeDataCtg(data models.Cottages, db *pgxpool.Pool) error {
